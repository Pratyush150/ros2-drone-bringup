...
These are figures computed by this package's pure-Python core, not simulator
screenshots. Each one is generated by calling the named module against the
committed YAML in `config/`.

![Top-down plot of the survey mission: transit legs, a lawnmower grid and an inspection orbit inside the geofence](docs/screenshots/mission-plan.png)
`config/example_mission.yaml` expanded by `drone_bringup.core.mission`: six mission items become 32 waypoints, drawn in local ENU with the zones from `config/example_geofence.yaml`. The lawnmower grid and the orbit are generated by the planner, not drawn by hand.

![Geofence check along a straight track, with breach points marked and a margin-to-boundary trace below](docs/screenshots/geofence-check.png)
`drone_bringup.core.geofence.Geofence.check_local` evaluated at 2 Hz along a 32-second straight track at 40 m and 8 m/s, against the fence in `config/example_geofence.yaml`. It flags the mast keep-out entry and the exit from the operating area, and predicts the keep-out entry 11.7 seconds before it happens.

![Terminal output of the preflight gate, the mission state machine transition log, a refused transition and two abort routes](docs/screenshots/preflight-state-machine.png)
`drone_bringup.core.state_machine` driven through a sortie: the preflight gate reports all seven failures at once rather than the first, the state machine logs every transition, an undeclared transition is refused with the list of legal targets, and an abort routes to RTL or to an immediate landing depending on the reason.
